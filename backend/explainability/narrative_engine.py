class NarrativeEngine:
    ...
    def generate(
        ...
        diagnostic_warnings = (diagnostics or {}).get("warnings", []) or []
        ...
        if reliability < 0.60:
            reliability_lines.append("Reliability: Recording quality is degraded, so calibrated confidence should be treated cautiously.")
        for warning in diagnostic_warnings:
            reliability_lines.append(f"Reliability: {warning}")
        ...
        human_summary = (
            f"{sections['Finding']} Confidence is {confidence:.1%} with reliability {reliability:.1%}. "
